...
一个基于 PyQt5 的业绩数据管理和可视化系统，支持多时期数据录入、姓名管理、操作保护和图表展示。
...
Win7 发布基线固定为 Win7 SP1 64 位、Python 3.8.20 和 Qt 5.15.8，详见 `environment-win7.yml`。
...
### 🗄️ 数据管理
- **多时期数据录入**: 按年月半年的时期管理
- **智能编号系统**: 每个时期内人员自动编号，支持上移/下移排序
- **统一姓名管理**: 下拉框选择，避免输入错误，保证数据一致性
- **自动增长计算**: 实时计算左区、右区和总增长百分比
- **🆕 上移下移优化**: 人员排序操作后姓名下拉框实时同步更新
- **🆕 空白姓名支持**: 新增行默认空白姓名，空数据自动跳过保存
- **未保存保护**: 切换时期、人员、页面或关闭窗口时提示保存、放弃或取消
- **即时输入校验**: 业绩与订单列在表格内直接提示格式错误
- **快捷键**: `Ctrl+S` 保存、`Insert` 新增、`Ctrl+Delete` 删除、`Alt+↑/↓` 排序、`F5` 重新加载

### 📊 数据可视化
- **个人趋势图**: 展示个人左区、右区和总业绩随时间的变化
- **时期对比图**: 使用横向分组柱状图比较同一时期不同人员的左右区业绩
- **显示设置**: 图表字体选项可折叠并自动记忆
