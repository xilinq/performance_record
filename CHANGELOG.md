...
- CSV 预检改为不可变快照与 SHA-256 校验，拒绝未来版本并兼容读取 v1/v2
- v3 严格要求完整业绩、总结和人员状态段，避免截断文件覆盖后静默丢数据
- 导入前创建唯一恢复快照，数据库提交结果与 CSV 镜像结果分别反馈
- 修复按人员保存后时期页旧值覆盖、六位小数精度丢失及重命名视图错乱
- 修复新增时期基于最早记录推算、本地化/64 位整数校验不一致和新时期增长率恒为零
- 新增数据目录可写检查与 Qt5 单实例锁；旧数据转换器统一使用 v3 CSV 编解码
- 自动镜像在启动及提交后从 SQLite 刷新；源码和测试入口补齐 Conda DLL 搜索路径，修复 `0xC06D007F`

## v1.3.0 (2026-08-27)

### 界面与操作流程

- 采用统一的现代浅色主题，规范按钮层级、表格样式、间距与字体回退
- 重组按时期、按人员工具栏和保存区，支持 `1024×768` 窗口使用
- 新增未保存状态保护，切换时期、人员、页面和关闭窗口前可保存、放弃或取消
- 新增数值单元格即时校验，以及保存、新增、删除、排序和刷新的键盘快捷键
- 人员时期改用结构化选择器，人员记录删除改为随批量保存原子提交
- 成功反馈移至状态栏，减少连续模态弹窗

### 图表与导入

- 图表筛选保留当前选择并避免重复绘图，显示设置可折叠并持久化
- 密集趋势图减少刻度与标签，时期人员比较改为横向分组柱状图
- CSV 导入新增无副作用预检，在覆盖前展示记录数量与兼容性提示

### Windows 7 打包

- 固定 Python 3.8.20、Qt 5.15.8 等 Win7 兼容依赖基线
- 新增版本化 `PerformanceApp.spec`，保持 onefile/windowed 并关闭 UPX
- 显式收集 Matplotlib 数据和 Qt Windows 平台插件
- 新增 `environment-win7.yml`，发布目标为 Win7 SP1 64 位及更高版本

## v1.2 (2025-01-03) ✨

### 稳定性修复

- 按时期和按人员保存改为完整事务，失败时不再留下部分数据
- CSV 导入在清空数据库前完成全量校验，并兼容缺少总增长率的旧格式
- CSV 备份新增格式版本、职级、排序和完整人员名册
...
- **数据库一致性**: 修改成功后，自动遍历数据库中所有相关记录并替换人名
  ...
  - 若新名字已存在且时期不冲突，则安全合并人员历史
  - 若新旧姓名存在相同时期记录，则拒绝覆盖并提示冲突时期
  - 自动重新计算修改后人员的增长率
- **UI实时同步**: 修改完成后自动刷新：
  - 按时期管理标签页中的数据表格
  - 所有姓名下拉框
